...
const fetchHeighPaidOrdersListByCity = async (city_to, country_to, to_country_short_name, page, isLocalOrders) => {
  //t posted-heigh-paid-international-orders
  let url = `${services.order.posted_international_orders}?city_to=${city_to}&page=${page}&is_local=${isLocalOrders?true:false}`;
  if (country_to) {
